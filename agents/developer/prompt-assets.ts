...
const DEVELOPER_RELEASE_1_0_4_BUNDLE_HASH =
  ...
  specification.templates.forEach((template, index) => {
    ...
    for (const field of ['id', 'name', 'filename', 'type', 'format', 'mediaType'] as const) {
      assertEqual(
        template[field],
        expected[field],
        'A identidade de um template de artifact não corresponde ao release 1.0.4.',
      );
    }
  });
}
...
export function parseDeveloperPromptAssets(
  ...
  try {
    ...
    assertEqual(
      bundleHash,
      DEVELOPER_RELEASE_1_0_4_BUNDLE_HASH,
      'O conteúdo do bundle não corresponde ao release Developer 1.0.4.',
    );
