...
export const NODE_TYPES = '/tmp/brq-node-types.d.ts';
...
export function validateOptionalPackage(content) {
  let document;
  try {
    document = JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(content));
  } catch {
  ...
  assertCondition(document.type === undefined || document.type === 'commonjs', 'PACKAGE_POLICY');
}
...
function typeScriptDiagnosticSummary(diagnostics) {
