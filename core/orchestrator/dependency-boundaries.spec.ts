...
describe('Orchestrator dependency boundaries', () => {
  it('depende somente dos entrypoints públicos dos agentes e de utilitários transversais', () => {
    ...
    for (const path of files) {
      for (const specifier of importedSpecifiers(readFileSync(path, 'utf8'))) {
        if (
          ...
          /^@brq\/(?:agent-runner|ai-provider|artifact-generator|knowledge-loader|prompt-builder|response-validator|prisma)(?:\/|$)/.test(
            specifier,
          ) ||
          /^@brq\/(?:product-owner-agent|developer-agent|qa-agent)\//.test(specifier) ||
          /(?:^|\/)(?:apps|prisma|repositories|execution-engine)(?:\/|$)/.test(specifier) ||
  ...
  it('não chama OpenAI, monta prompts, gera artifacts ou implementa retry', () => {
    const source = productionTypeScriptFiles(MODULE_ROOT)
      .map((path) => readFileSync(path, 'utf8'))
      .join('\n');
    expect(source).not.toMatch(
      /\b(?:OpenAI|AIProvider|PromptBuilder|ArtifactGenerator|ResponseValidator|KnowledgeLoader|PrismaClient)\b/,
    );
    ...
    expect(source).not.toMatch(/\b(?:retry|backoff)\b/i);
  });

  it('declara apenas as dependências aprovadas e um único entrypoint', () => {
  ...
  it('não cria alias público para deep imports', () => {
    ...
    expect(tsconfig.compilerOptions?.paths?.['@brq/orchestrator/*']).toBeUndefined();
  });
});
